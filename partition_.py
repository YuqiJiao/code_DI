...
########################################################################################################
        ...
        for it in range(1):
            ...
            active_keep = \
                self.get_trimmed_set(
                    ...
                    labels, beta, active_num, label_indices)#active_labels
            indices_to_keep = np.copy(self.active)
            ...
            if len(self.y_train[0]) <= 2:
                if active_keep.sum() <= 0.1*len(self.x_train):
                    print("break")

                    break
            else:
                if active_keep.sum() <= 0.001*len(self.x_train):
                    print("break")
        ...
        loss = self.losses
        if len(self.y_train[0]) <= 2:
            loss = np.zeros((len(self.x_train),))
        else:
            loss = self.losses
        ...
        if len(self.y_train[0]) > 2:
            ...
            for label in np.unique(labels):
                ...
                min_025_indices = indices[
                    sorted_indices_for_label[:int(0.025 * len(sorted_indices_for_label))]]
