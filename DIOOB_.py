...
from partition_ import Partition



class DIOOB(DataEvaluator):

    def __init__(self, outer_iter=800, port=0.1, mut=1.7, port_delet=0.3, ratio = 0.02,random_state: RandomState = None, *args,
                 **kwargs):
    ...
    def filter_noise(self,
        ...
        for b in range(1):
            ...
            for e in range(boost):
                if len(self.x_train) > 800:
                    if sum(active)/len(active) <= 0.1:
                        break
                else:
                    if sum(active)/len(active) <= 0.001:
                        break
        ...
        for idx in range(len(learners)):
            ...
            y_pred_val = learners[idx].predict(self.x_valid)
            y_pred_val = torch.Tensor(y_pred_val)
            predicted_val = torch.argmax(y_pred_val, dim=1)
            predicted_val = predicted_val.cpu().numpy()
            val_y_ = np.argmax(self.y_valid, axis = 1)
            val_y_ = val_y_.cpu().numpy()
            c_num_[idx] = sum(predicted_val == val_y_)
    ...
    def train_data_values(self, *args, **kwargs):
        ...
        for idx in tqdm.tqdm(range(self.outer_iter)):
            ...
            distance = torch.nn.functional.cross_entropy(y_train_hat.cpu(), self.y_train[indices_out], reduction='none')
