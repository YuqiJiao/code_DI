...
def weight_accuracy(evaluator: DataEvaluator, fetcher: DataFetcher)-> dict[str, float]:
    """
    Evaluate the weight accuracy of the data evaluator.
    """
    data_values = evaluator.data_values
    weight = np.array(data_values).reshape(-1,)
    ...
    weight = ((weight - min_val) / (max_val - min_val)) * (new_max - new_min) + new_min
