...
def mix_labels_train(fetcher: DataFetcher, noise_rate: float = 0.2) -> dict[str, np.ndarray]:
    """Mixes y_train labels of a DataFetcher, adding noise to training data only.
...
def add_gauss_noise_to_train(
        fetcher: DataFetcher, noise_rate: float = 0.2, mu: float = 0.0, sigma: float = 1.0
) -> dict[str, np.ndarray]:
    """Add Gaussian noise to training covariates (x_train).
    ...
    mu : float, optional
        Mean of the Gaussian noise, by default 0
    sigma : float, optional
        Standard deviation of the Gaussian noise, by default 1
    ...
    dict[str, np.ndarray]
        ...
        - **"noisy_train_indices"** -- Indices of training data set with added noise
    """
    rs = check_random_state(fetcher.random_state)
...
class NoiseFunc(FuncEnum):
    ...
    MIX_LABELS_TRAIN = FuncEnum.wrap(mix_labels_train)
    ADD_GAUSS_NOISE_TO_TRAIN = FuncEnum.wrap(add_gauss_noise_to_train)
