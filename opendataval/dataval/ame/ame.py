...
class AME(DataEvaluator):
    ...
    def evaluate_data_values(self) -> np.ndarray:
        ...
        # 
        # norm_subsets = ((self.subsets.T - np.mean(self.subsets, axis=1)) / (
        #             np.mean(self.subsets, axis=1) * (1 - np.mean(self.subsets, axis=1)))).T
        # norm_subsets[np.isnan(norm_subsets)] = 0  # For when all elements are the same
        # y_dv_ame_list = y_dv_ame_list - np.mean(y_dv_ame_list)
        
        centered_perf = self.performance - np.mean(self.performance)
        # centered_perf = self.performance
...
class BaggingEvaluator(DataEvaluator):
    ...
    def train_data_values(self, *args, **kwargs):
        ...
        # self.subsets = self.random_state.binomial(1, 0.5, size=sample_dim)
