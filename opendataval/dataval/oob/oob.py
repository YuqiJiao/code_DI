...
class DataOob(DataEvaluator):
    ...
    def input_data(
        ...
        [*self.label_dim] = (1,) if self.y_train.ndim == 1 else self.y_train[0].shape####得到标签的维度or数量
        ...
        #self.oob_pred 和 self.oob_indices：这些是用于存储数据集成结果和相关信息的属性。self.oob_pred 初始化为空的张量（tensor），
        # 用于存储数据集成的预测结果。self.oob_indices 是一个未知类型的对象，可能用于存储数据点的索引信息。
        # return self：这是一个返回语句，它返回当前类的实例，以便可以链式调用其他方法。
